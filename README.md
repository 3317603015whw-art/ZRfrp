...
ZRfrp 是一套面向 frp 的多用户控制平台，包含 Windows Desktop 客户端和 Linux Server 控制平面。历史个人使用版冻结在 [`personal-manager`](https://github.com/masZR-art/ZRfrp/tree/personal-manager) 分支。
...
- 双端账号：客户登录后动态取得短期会话与 frp Token。
- 角色面板：管理员管理节点、账号和额度，客户查看自己的流量用量。
- 主控节点：通过心跳监控子节点，并可远程控制节点服务。
- 自动更新：Desktop 与 Server 都可检查 GitHub Release 并一键更新。
...
支持 Linux x64 与 arm64。服务端发布包内置运行环境，不需要预先安装 .NET。安装后会输出面板密码、客户端 API Key 和 frp Token。详细说明见 [ZRfrp.Server/README.md](ZRfrp.Server/README.md)。
